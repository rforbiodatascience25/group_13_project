...
ggsave(here("results/figures/correlation_matrix.png"), 
       width = 7, 
       height = 5)
```
...
```{r}
model <- lm(molecular_weight ~ length, 
            data = clean_df)
summary(model)
...
ggplot(clean_df, aes(x = length, 
  ...
  theme_minimal() +
  labs(title = "Molecular Weight vs Length", 
       x ="Length (in amino acids)", 
       y="Molecular Weight",
       subtitle = paste("R-squared:", round(summary(model)$r.squared, 3)))

ggsave(here("results/figures/lm_plot.png"), 
       width = 7, 
       height = 5)
```
...
```{r}
clean_df <- clean_df |> 
  mutate(outlier_score = rstandard(model))
...
comparison <- outlier_aa |>
  mutate(type = "outlier") |>
  bind_rows(
    aa_means |> mutate(Protein_ID = "Dataset Mean", 
                       type = "mean")
  )
...
outlier_long <- outlier_aa |>
  pivot_longer(cols = starts_with("aa_"), 
               names_to = "AA", 
               values_to = "Value")

ggplot(outlier_long, aes(x = AA, 
                         y = Value, 
                         fill = Protein_ID)) +
...
aa_weights |>
  ...
  scale_fill_gradient(low = "lightblue", 
                      high = "darkblue") +
  geom_col(aes(fill = `Molecular Weight`), 
           alpha=0.9) +
  coord_flip() +
  theme_minimal() +
  labs(
...
diff_long <- outliers |>
  ...
  pivot_longer(
    ...
    names_to = "AA",
    values_to = "OutlierValue"
  ) |>
  mutate(AA_Letter = stringr::str_remove(AA, "aa_")) |>
  left_join(
    aa_means |>
      pivot_longer(
        cols = everything(),
  ...
  left_join(aa_weights, 
            by = c("AA_Letter" = "Letter")) |>
  select(-AA_Letter, -Name, -Abbr)

ggplot(diff_long, aes(x = reorder(AA, `Molecular Weight`, descending = TRUE),
                      y = Difference,
                      fill = Difference > 0)) +
  geom_col() +
  coord_flip() +
  ...
  theme_minimal() +
  labs(
    title = "AA Composition Deviation from Dataset Mean (Ordered by AA weight)",
    x = "Amino Acid",
    y = "Difference (Outlier − Mean)"
  )

ggsave(here("results/figures/outlier_comp_deviation_plot.png"), 
       width = 7, 
       height = 5)
...
```{r}
outliers <- clean_df |> 
  filter(abs(rstandard(model)) > 3)

outlier_comp <- comp_long2 |> 
  filter(Protein_ID %in% outliers$Protein_ID)

ggplot(outlier_comp, aes(x = AA, 
                         y = Weight_contrib, 
                         fill = Protein_ID)) +
  geom_col(position = "dodge") +
  coord_flip() +
