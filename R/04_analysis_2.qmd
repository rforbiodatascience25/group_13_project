...
ggsave(here("results/figures/correlation_matrix.png"), width = 7, height = 5)
```
...
ggplot(clean_df, aes(x = length, y = molecular_weight)) +
  ...
  geom_text(
    data = clean_df |> filter(Protein_ID %in% c("P42260", "Q9C8E7")),
...
ggsave(here("results/figures/lm_plot.png"), width = 7, height = 5)
```
...
diff_long <- outliers |>
  ...
  left_join(aa_weights, by = c("AA_Letter" = "Letter")) |>
...
ggsave(here("results/figures/outlier_comp_deviation_plot.png"), width = 7, height = 5)
