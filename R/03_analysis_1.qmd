...
Defining classes. Based on knowledge provided by Ákos, we will split the receptors into 3 classes: Glutamate, Cys-loop and Other receptors (which includes 2 ionotropic receptors not part of the other 2 superfamilies).

```{r}
clean_classed_df <- clean_df |>
  mutate(
    Receptor_class = case_when(
      str_detect(Protein_Name, 
                 regex("glutamate", ignore_case = TRUE)) ~ 
        "Ionotropic glutamate receptor",

      str_detect(Protein_Name, 
                 regex("^Ionotropic receptor", ignore_case = TRUE)) ~
...
Selecting only the relevant columns for this predicition
...
ggplot(pca_juiced, aes(PC1, 
                       PC2, 
                       color = Receptor_class)) +
...
ggsave(here("results/figures/pca_plot.png"), 
       width = 7, 
       height = 5)
```
...
```{r}
set.seed(123)
split  <- initial_split(prediction_df, 
                        prop = 0.8, 
                        strata = Receptor_class)
...
ggplot(metrics2,
       aes(x = metric_label, 
           ...
           fill = direction)) +
  geom_col(alpha = 0.6) +
  geom_text(aes(label = estimate_lbl), 
            vjust = -0.5, 
            size = 4) +
  scale_fill_manual(
    name = "Interpretation",
...
```{r}
ggsave(here("results/figures/metrics_plot.png"), 
       width = 7, 
       height = 5)
```
...
ggsave(here("results/figures/confusion_matrix_plot.png"), 
       width = 7, 
       height = 5)
```
...
importance_df <- data.frame(
  ...
  mutate(Variable = str_remove(Variable, "aa_")) |> 
...
ggsave(here("results/figures/aa_importance_plot.png"), 
       width = 7, 
       height = 5)
```

Look at the most discriminative aminoacids and see if they predominate in one of the two receptor classes.

```{r}
prediction_df |> 
  group_by(Receptor_class) |> 
  summarise(across(starts_with("aa_"), median)) |> 
  select(Receptor_class, aa_V, aa_G, aa_W, aa_S, aa_P, aa_Q, aa_C) |> 
  filter(Receptor_class != "Other ionotropic receptor")
