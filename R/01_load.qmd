...
dir.create(here("data/_raw"), showWarnings = FALSE)
output_file <- here("data/_raw/neurotransmitter-receptors.zip")
...
```{r}
unzip(zipfile = output_file, exdir = here("data/_raw"))

raw_file <- here("data/_raw/preproteins.csv")

clean_df <- readr::read_csv(raw_file)

```
### Additional Data
Because we refer to it later, we also load data for the properties of the aminoacids


TODO -> move the download and extract to utils!
```{r}
FORCE_DOWNLOAD <- FALSE

target_url <- "https://www.kaggle.com/api/v1/datasets/download/alejopaullier/aminoacids-physical-and-chemical-properties"


dir.create(here("data"), showWarnings = FALSE)
dir.create(here("data/_raw"), showWarnings = FALSE)
output_file <- here("data/_raw/aa_properties.zip")
...
raw_file <- here("data/_raw/aminoacids.csv")

aa_properties <- readr::read_csv(raw_file)
aa_properties
```
Store the relevant information for us in another file for easy retrieval

```{r}
aa_weights <- aa_properties |>
  select(Name, Abbr, Letter, `Molecular Weight`)

readr::write_csv(aa_weights,
                 here("data/03_aa_weights.csv"))
...
readr::write_csv(dirty_df_meta,
                 here("data/01_dat_meta_load.csv"))

readr::write_csv(dirty_df_feature,
                 here("data/01_dat_feature_load.csv"))
