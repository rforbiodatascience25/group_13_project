...
```{r}
ggsave(here("results/figures/species_split.png"),
       width = 5,
       height = 5)
```
...
#   mutate(words = str_split(tolower(Function), "[[:space:][:punct:]]+")) |>
#   unnest(words) |>
#   filter(words != "") |>
#   filter(!words %in% c("of","as","a","part","that","the","function", "n", "non", "5")) |>
...
```{r}
ggsave(here("results/figures/sequence_lengths.png"),
       width = 7,
       height = 5)
```
...
```{r}
ggsave(here("results/figures/missingness_matrix.png"),
       width = 7,
       height = 5)
```
...
```{r}
ggsave(here("results/figures/boxplots_outliers_dirty.png"),
       width = 7,
       height = 5)
```
...
```{r}
ggsave(here("results/figures/boxplots_outliers_clean.png"),
       width = 7,
       height = 5)
```
